...
func (c *Config) validate() error {
	...
	for i, s := range c.Symbols {
		...
		if s.FastPeriod < 2 {
			return fmt.Errorf("symbols[%d]: fast_period must be >= 2", i)
		}
		if s.SlowPeriod < 2 {
			return fmt.Errorf("symbols[%d]: slow_period must be >= 2", i)
		}
		if s.FastPeriod >= s.SlowPeriod {
			return fmt.Errorf("symbols[%d]: fast_period must be less than slow_period", i)
		}
