...
// the user to start from — typically generated by Close Position or
// Replace menu entries. nonce changes on every prefill so the OrderForm
// re-applies even when the same values are pushed twice.
//
// Replace flow: when replaceOrderId is set, submission carries it on
// SingleOrderPlan so the saga atomically cancels the original order
// and places this one in its stead. tif and displayQuantity preserve
// the original order's settings on replace; for Close-Position prefills
// they're omitted and the form falls back to its standard defaults.
...
export function OrderForm({
  ...
  const [replaceOrderId, setReplaceOrderId] = useState<string>("");
  ...
  useEffect(() => {
    ...
    if (prefill.orderType === "LIMIT") {
      ...
      setTif(prefill.tif ?? "DAY");
    } else {
      setPrice("");
      setTif(prefill.tif ?? "IOC");
    }
    if (prefill.displayQuantity && prefill.displayQuantity > 0) {
      setIceberg(true);
      setDisplayQty(prefill.displayQuantity);
    } else {
      setIceberg(false);
    }
    setReplaceOrderId(prefill.replaceOrderId ?? "");
  }, [prefill?.nonce]);
  ...
  function handleSubmit() {
    ...
    if (isSingle) {
      ...
      const isReplace = replaceOrderId !== "";
      const rows = [
      ...
      if (isReplace) {
        rows.push({ label: "Replaces", value: replaceOrderId.slice(0, 8) + "…" });
      }
      setPending({
        request: {
          $typeName: "saga.v1.PlaceSagaRequest",
        ...
        heading: `${isReplace ? "Replace" : verb} ${qty} ${symbol}${useIceberg ? " (iceberg)" : ""}`,
        color: ACTION_MAP[action].color,
        rows,
        successTitle: isReplace ? "Order replaced" : "Order placed",
        successMessage: `${isReplace ? "Replaced" : verb} ${qty} ${symbol}`,
        failureTitle: isReplace ? "Replace failed" : "Order failed",
      });
      return;
    }
  ...
  return (
    <Card withBorder>
      <Stack gap="sm">
        <Title order={5}>{replaceOrderId ? "Replace Order" : "Place Order"}</Title>
        {replaceOrderId && (
          <Alert
            color="blue"
            withCloseButton
            onClose={() => setReplaceOrderId("")}
            title="Replacing existing order"
          >
            <Text size="xs">
              Submitting will atomically cancel order{" "}
              <Code>{replaceOrderId.slice(0, 8)}…</Code> and place this new one
              in its stead.
            </Text>
          </Alert>
        )}
        ...
        >
          {isSingle && `${replaceOrderId ? "Replace" : ACTION_MAP[action].verb} ${symbol}`}
